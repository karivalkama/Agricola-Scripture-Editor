...
class USXChapterProcessor: USXContentProcessor
{
	...
	static func createChapterParser(caller: XMLParserDelegate, index: Int, targetPointer: UnsafeMutablePointer<[Chapter]>, using errorHandler: @escaping (USXParseError) -> ()) -> USXContentParser<Chapter, Section>
	{
	...
	func getParser(_ caller: USXContentParser<Chapter, Section>, forElement elementName: String, attributes: [String : String], into targetPointer: UnsafeMutablePointer<[Section]>, using errorHandler: @escaping (USXParseError) -> ()) -> (XMLParserDelegate, Bool)?
	{
	...
	func getCharacterParser(_ caller: USXContentParser<Chapter, Section>, into targetPointer: UnsafeMutablePointer<[Section]>, using errorHandler: (USXParseError) -> ()) -> XMLParserDelegate?
	{
		// Character parsing is only handled below para level
		return nil
	}
	
	func generate(from content: [Section], using errorHandler: (USXParseError) -> ()) -> Chapter?
	{
